...
algo-puct = PUCT
opt-c-puct = Exploração PUCT (c)
opt-c-puct-hint = Constante de exploração PUCT — mais alto explora mais. Predefinição 1.5.
opt-feat-adapt = NRPA espaço-features
...
opt-feat-table = Manter tabela de política (φ-B)
opt-feat-table-hint = Manter a tabela de política one-hot ao lado de θ·φ (φ-B). Desativado → φ-A só cabeça. Só com espaço-features.
opt-feat-warm = Init. a quente da cabeça
opt-feat-warm-hint = Init. a quente θ₀ = escala·cabeça, reproduz o prior no passo 0. Desativado → a frio (θ₀ = 0). Só com espaço-features.
opt-feat-lambda = Decaimento L2 λ da cabeça
opt-feat-lambda-hint = Decaimento L2 θ ← (1−λ)θ após cada adaptação. Predefinição 0 = desativado. Só com espaço-features.
opt-feat-clamp = Limite C da cabeça
opt-feat-clamp-hint = Limita |θ_j| ≤ C após cada adaptação. Predefinição 0 = desativado. Só com espaço-features.
opt-feat-norm = Normalizar φ
opt-feat-norm-hint = Normaliza (L2) cada φ em cache para comprimento 1. Quebra a reprodução a quente; para varrimentos a frio. Só com espaço-features.
...
search-configure = ⚙ Configurar pesquisa…
search-configure-hint = Motor, opções, prior, critérios de paragem
stop-criteria = Critérios de paragem
stop-after = Parar após
stop-at-score = Parar na pontuação
stop-nodes = Parar após nós
adv-header = Avançado
adv-threads = Threads
adv-max-memory = Memória máx.
adv-ignore-overflow = Ignorar transbordo da grelha
adv-experimental = Motores e opções experimentais
adv-experimental-hint = Revela métodos de pesquisa e ajustes de laboratório (PUCT, macros, espaço de características neuronal). Desativado por predefinição.
setup-cli-command = Comando CLI equivalente
setup-copy-command = Copiar comando
setup-running-note = Uma pesquisa está a decorrer — os critérios de paragem aplicam-se já; outras alterações na próxima execução.
setup-start-search = ▶  Iniciar pesquisa
